package com.easemob.chatuidemo.chatuidemo.db;
...
import com.easemob.chatuidemo.chatuidemo.domain.InviteMessage;
...
public class DemoDBManager {
    ...
    private com.easemob.chatuidemo.chatuidemo.db.DbOpenHelper dbHelper;
    
    void onInit(Context context){
        dbHelper = com.easemob.chatuidemo.chatuidemo.db.DbOpenHelper.getInstance(context);
    }
    ...
    synchronized public void saveContactList(List<User> contactList) {
        ...
        if (db.isOpen()) {
            db.delete(com.easemob.chatuidemo.chatuidemo.db.UserDao.TABLE_NAME, null, null);
            for (User user : contactList) {
                ContentValues values = new ContentValues();
                values.put(com.easemob.chatuidemo.chatuidemo.db.UserDao.COLUMN_NAME_ID, user.getUsername());
                if(user.getNick() != null)
                    values.put(com.easemob.chatuidemo.chatuidemo.db.UserDao.COLUMN_NAME_NICK, user.getNick());
                if(user.getAvatar() != null)
                    values.put(com.easemob.chatuidemo.chatuidemo.db.UserDao.COLUMN_NAME_AVATAR, user.getAvatar());
                db.replace(com.easemob.chatuidemo.chatuidemo.db.UserDao.TABLE_NAME, null, values);
            }
        }
    }
    ...
    synchronized public Map<String, User> getContactList() {
        ...
        if (db.isOpen()) {
            Cursor cursor = db.rawQuery("select * from " + com.easemob.chatuidemo.chatuidemo.db.UserDao.TABLE_NAME /* + " desc" */, null);
            while (cursor.moveToNext()) {
                String username = cursor.getString(cursor.getColumnIndex(com.easemob.chatuidemo.chatuidemo.db.UserDao.COLUMN_NAME_ID));
                String nick = cursor.getString(cursor.getColumnIndex(com.easemob.chatuidemo.chatuidemo.db.UserDao.COLUMN_NAME_NICK));
                String avatar = cursor.getString(cursor.getColumnIndex(com.easemob.chatuidemo.chatuidemo.db.UserDao.COLUMN_NAME_AVATAR));
                User user = new User();
    ...
    synchronized public void deleteContact(String username){
        SQLiteDatabase db = dbHelper.getWritableDatabase();
        if(db.isOpen()){
            db.delete(com.easemob.chatuidemo.chatuidemo.db.UserDao.TABLE_NAME, com.easemob.chatuidemo.chatuidemo.db.UserDao.COLUMN_NAME_ID + " = ?", new String[]{username});
        }
    }
    ...
    public void setDisabledGroups(List<String> groups){
        setList(com.easemob.chatuidemo.chatuidemo.db.UserDao.COLUMN_NAME_DISABLED_GROUPS, groups);
    }
    
    public List<String>  getDisabledGroups(){       
        return getList(com.easemob.chatuidemo.chatuidemo.db.UserDao.COLUMN_NAME_DISABLED_GROUPS);
    }
    
    public void setDisabledIds(List<String> ids){
        setList(com.easemob.chatuidemo.chatuidemo.db.UserDao.COLUMN_NAME_DISABLED_IDS, ids);
    }
    
    public List<String> getDisabledIds(){
        return getList(com.easemob.chatuidemo.chatuidemo.db.UserDao.COLUMN_NAME_DISABLED_IDS);
    }
    ...
    synchronized private void setList(String column, List<String> strList){
        ...
        if (db.isOpen()) {
            ...
            db.update(com.easemob.chatuidemo.chatuidemo.db.UserDao.PREF_TABLE_NAME, values, null,null);
        }
    }
    
    synchronized private List<String> getList(String column){
        SQLiteDatabase db = dbHelper.getReadableDatabase();
        Cursor cursor = db.rawQuery("select " + column + " from " + com.easemob.chatuidemo.chatuidemo.db.UserDao.PREF_TABLE_NAME,null);
	...
	synchronized public void saveRobotList(List<RobotUser> robotList) {
		SQLiteDatabase db = dbHelper.getWritableDatabase();
		if (db.isOpen()) {
			db.delete(com.easemob.chatuidemo.chatuidemo.db.UserDao.ROBOT_TABLE_NAME, null, null);
			for (RobotUser item : robotList) {
				ContentValues values = new ContentValues();
				values.put(com.easemob.chatuidemo.chatuidemo.db.UserDao.ROBOT_COLUMN_NAME_ID, item.getUsername());
				if (item.getNick() != null)
					values.put(com.easemob.chatuidemo.chatuidemo.db.UserDao.ROBOT_COLUMN_NAME_NICK, item.getNick());
				if (item.getAvatar() != null)
					values.put(com.easemob.chatuidemo.chatuidemo.db.UserDao.ROBOT_COLUMN_NAME_AVATAR, item.getAvatar());
				db.replace(com.easemob.chatuidemo.chatuidemo.db.UserDao.ROBOT_TABLE_NAME, null, values);
			}
		}
	}
	...
	synchronized public Map<String, RobotUser> getRobotList() {
		...
		if (db.isOpen()) {
			Cursor cursor = db.rawQuery("select * from " + com.easemob.chatuidemo.chatuidemo.db.UserDao.ROBOT_TABLE_NAME, null);
			if(cursor.getCount()>0){
				users = new HashMap<String, RobotUser>();
			};
			while (cursor.moveToNext()) {
				String username = cursor.getString(cursor.getColumnIndex(com.easemob.chatuidemo.chatuidemo.db.UserDao.ROBOT_COLUMN_NAME_ID));
				String nick = cursor.getString(cursor.getColumnIndex(com.easemob.chatuidemo.chatuidemo.db.UserDao.ROBOT_COLUMN_NAME_NICK));
